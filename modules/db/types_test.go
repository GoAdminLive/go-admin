...
func testPG(t *testing.T, port string) {
	...
	_, err = conn.Exec(
		`INSERT INTO public.` + typeTesttableName + `(
...
func testConnDSN(driver, dsn string) Connection {
	return GetConnectionByDriver(driver).InitDB(
