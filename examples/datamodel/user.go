...
func GetUserTable(ctx *context.Context) (userTable table.Table) {
	...
	info.AddField("Gender", "gender", db.Tinyint).FieldDisplay(
	...
	).FieldEditAble(editType.Switch).FieldEditOptions(
		types.FieldOptions{
			{
				Value: "0",
				Text:  "👦",
	...
	).FieldFilterable(types.FilterType{FormType: form.SelectSingle}).FieldFilterOptions(
		types.FieldOptions{
			{
				Value: "0",
				Text:  "men",
	...
	info.AddColumn(
		"Personality", func(value types.FieldModel) interface{} {
			return "handsome"
		},
	)
	info.AddColumnButtons(
		ctx, "see more", types.GetColumnButton(
			"see more", icon.Info,
			action.PopUp(
				"/see/more/example", "see more",
	...
	info.AddField("Avatar", "avatar", db.Varchar).FieldDisplay(
	...
	info.AddActionButton(
		ctx, "Audit", action.Ajax(
			"/admin/audit",
			func(ctx *context.Context) (success bool, msg string, data interface{}) {
	...
	info.AddButton(
		ctx, "popup", icon.Terminal, action.PopUp(
			"/admin/popup", "Popup Example",
			func(ctx *context.Context) (success bool, msg string, data interface{}) {
				return true, "", "<h2>hello world</h2>"
			},
		),
	...
	info.AddButton(
		ctx, "iframe", icon.Tv, action.PopUpWithIframe(
			"/admin/iframe", "Iframe Example",
			action.IframeData{Src: "/admin/info/authors"}, "900px", "560px",
		),
	)
	info.AddButton(
		ctx, "form", icon.Folder, action.PopUpWithForm(
			action.PopUpData{
				Id:     "/admin/popup/form",
	...
	info.AddButton(
		ctx, "ajax", icon.Android, action.Ajax(
			"/admin/ajax",
			func(ctx *context.Context) (success bool, msg string, data interface{}) {
				return true, "success", ""
			},
	...
	formList.AddField("Country", "country", db.Tinyint, form.SelectSingle).
		...
		).FieldDefault("0").FieldOnChooseAjax(
		"city", "/choose/country",
...
<input type="text" name="{{.Field}}" value="{{.Value}}" class="form-control {{.Field}}" placeholder="please input {{.Head}}">`,
	...
	formList.AddField("Custom Field", "role", db.Varchar, form.Text).
		FieldPostFilterFn(
			func(value types.PostFieldModel) interface{} {
	...
	userTable.GetForm().SetTabGroups(
		types.
