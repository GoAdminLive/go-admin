...
func (s *SystemTable) GetManagerTable(ctx *context.Context) (managerTable Table) {
	...
	info.SetTable("goadmin_users").
		...
		SetDeleteFn(
			func(idArr []string) error {

				var ids = interfaces(idArr)

				_, txErr := s.connection().WithTransaction(
					func(tx *sql.Tx) (e error, i map[string]interface{}) {
	...
	formList.AddField(lg("role"), "role_id", db.Varchar, form.Select).
		...
		template.HTML(lg("no corresponding options?")) +
	...
	formList.SetUpdateFn(
		func(values form2.Values) error {
		...
		FieldDisplay(
			func(model types.FieldModel) interface{} {
				permissionModel, _ := s.table("goadmin_user_permissions").
					...
					LeftJoin(
						"goadmin_permissions", "goadmin_permissions.id", "=", "goadmin_user_permissions.permission_id",
					).
					Where("user_id", "=", model.ID).
					All()
...
func (s *SystemTable) GetNormalManagerTable(ctx *context.Context) (managerTable Table) {
	...
	formList.AddField("ID", "id", db.Int, form.Default).FieldDisplayButCanNotEditWhenUpdate().FieldDisableWhenCreate()
	formList.AddField(
		lg("Name"), "username", db.Varchar, form.Text,
	).FieldHelpMsg(template.HTML(lg("use for login"))).FieldMust()
	formList.AddField(
		lg("Nickname"), "name", db.Varchar, form.Text,
	).FieldHelpMsg(template.HTML(lg("use to display"))).FieldMust()
	formList.AddField(lg("Avatar"), "avatar", db.Varchar, form.File)
	formList.AddField(lg("password"), "password", db.Varchar, form.Password).
		FieldDisplay(
...
func (s *SystemTable) GetPermissionTable(ctx *context.Context) (permissionTable Table) {
	...
	info.AddField(lg("method"), "http_method", db.Varchar).FieldDisplay(
		func(value types.FieldModel) interface{} {
			if value.Value == "" {
				return "All methods"
			}
	...
	formList.AddField(
		lg("slug"), "slug", db.Varchar, form.Text,
	).FieldHelpMsg(template.HTML(lg("should be unique"))).FieldMust()
	formList.AddField(lg("method"), "http_method", db.Varchar, form.Select).
		...
		FieldPostFilterFn(
			func(model types.PostFieldModel) interface{} {
...
func (s *SystemTable) GetRolesTable(ctx *context.Context) (roleTable Table) {
	...
	formList.SetInsertFn(
		func(values form2.Values) error {
			...
			_, txErr := s.connection().WithTransaction(
				func(tx *sql.Tx) (e error, i map[string]interface{}) {
					role, createRoleErr := models.Role().WithTx(tx).SetConn(s.conn).New(
...
func (s *SystemTable) GetOpTable(ctx *context.Context) (opTable Table) {
	...
	info.AddField(lg("user"), "name", db.Varchar).FieldJoin(
		types.Join{
	...
	info.AddSelectBox(
		ctx, language.Get("method"), types.FieldOptions{
			{
				Value: "GET",
				Text:  "GET",
			},
...
func (s *SystemTable) GetMenuTable(ctx *context.Context) (menuTable Table) {
	...
	if len(allMenuIDs) > 0 {
		...
		for i := 0; i < len(allMenus); i++ {
			parentIDOptions = append(
				parentIDOptions, types.FieldOption{
...
func (s *SystemTable) GetSiteTable(ctx *context.Context) (siteTable Table) {
	siteTable = NewDefaultTable(
		ctx, DefaultConfigWithDriver(config.GetDatabases().GetDefault().Driver).
			SetOnlyUpdateForm().
			SetGetDataFun(
				func(params parameter.Parameters) (i []map[string]interface{}, i2 int) {
	...
	for k, t := range language.Langs {
		langOps[k] = types.FieldOption{
			Text:  lgWithConfigScore(t, "language"),
			Value: t,
		}
	}
	...
	formList.AddField(
		lgWithConfigScore("allow delete operation log"), "allow_del_operation_log", db.Varchar, form.Switch,
	).
		FieldOptions(
			types.FieldOptions{
				{
	...
	formList.AddField(
		lgWithConfigScore("hide config center entrance"), "hide_config_center_entrance", db.Varchar, form.Switch,
	).
		FieldOptions(
			types.FieldOptions{
				{
	...
	formList.AddField(
		lgWithConfigScore("logger rotate max backups"), "logger_rotate_max_backups", db.Varchar, form.Number,
	).
	...
	formList.AddField(
		lgWithConfigScore("logger rotate encoder encoding"), "logger_encoder_encoding", db.Varchar,
		form.SelectSingle,
	).
	...
	formList.AddField(
		lgWithConfigScore("logger rotate encoder time key"), "logger_encoder_time_key", db.Varchar, form.Text,
	).
		FieldDisplay(defaultFilterFn("ts"))
	formList.AddField(
		lgWithConfigScore("logger rotate encoder level key"), "logger_encoder_level_key", db.Varchar, form.Text,
	).
		FieldDisplay(defaultFilterFn("level"))
	formList.AddField(
		lgWithConfigScore("logger rotate encoder name key"), "logger_encoder_name_key", db.Varchar, form.Text,
	).
		FieldDisplay(defaultFilterFn("logger"))
	formList.AddField(
		lgWithConfigScore("logger rotate encoder caller key"), "logger_encoder_caller_key", db.Varchar, form.Text,
	).
		FieldDisplay(defaultFilterFn("caller"))
	formList.AddField(
		lgWithConfigScore("logger rotate encoder message key"), "logger_encoder_message_key", db.Varchar, form.Text,
	).
		FieldDisplay(defaultFilterFn("msg"))
	formList.AddField(
		lgWithConfigScore("logger rotate encoder stacktrace key"), "logger_encoder_stacktrace_key", db.Varchar,
		form.Text,
	).
		FieldDisplay(defaultFilterFn("stacktrace"))

	formList.AddField(
		lgWithConfigScore("logger rotate encoder level"), "logger_encoder_level", db.Varchar,
		form.SelectSingle,
	).
		FieldOptions(
			types.FieldOptions{
				{
					Text:  lgWithConfigScore("capital"),
					Value: "capital",
				},
				{
					Text:  lgWithConfigScore("capitalcolor"),
					Value: "capitalColor",
				},
				{
					Text:  lgWithConfigScore("lowercase"),
					Value: "lowercase",
				},
				{
					Text:  lgWithConfigScore("lowercasecolor"),
					Value: "color",
				},
			},
		).FieldDisplay(defaultFilterFn("capitalColor"))
	formList.AddField(
		lgWithConfigScore("logger rotate encoder time"), "logger_encoder_time", db.Varchar,
		form.SelectSingle,
	).
		FieldOptions(
			types.FieldOptions{
				{
					Text:  "ISO8601(2006-01-02T15:04:05.000Z0700)",
					Value: "iso8601",
				},
				{
					Text:  lgWithConfigScore("millisecond"),
					Value: "millis",
				},
				{
					Text:  lgWithConfigScore("nanosecond"),
					Value: "nanos",
				},
				{
					Text:  "RFC3339(2006-01-02T15:04:05Z07:00)",
					Value: "rfc3339",
				},
				{
					Text:  "RFC3339 Nano(2006-01-02T15:04:05.999999999Z07:00)",
					Value: "rfc3339nano",
				},
			},
		).FieldDisplay(defaultFilterFn("iso8601"))
	formList.AddField(
		lgWithConfigScore("logger rotate encoder duration"), "logger_encoder_duration", db.Varchar,
		form.SelectSingle,
	).
		FieldOptions(
			types.FieldOptions{
				{
					Text:  lgWithConfigScore("seconds"),
					Value: "string",
				},
				{
					Text:  lgWithConfigScore("nanosecond"),
				...
				{
					Text:  lgWithConfigScore("microsecond"),
					Value: "ms",
				},
			},
		).FieldDisplay(defaultFilterFn("string"))
	formList.AddField(
		lgWithConfigScore("logger rotate encoder caller"), "logger_encoder_caller", db.Varchar,
		form.SelectSingle,
	).
		FieldOptions(
			types.FieldOptions{
				{
					Text:  lgWithConfigScore("full path"),
					Value: "full",
				},
				{
					Text:  lgWithConfigScore("short path"),
					Value: "short",
				},
			},
	...
	formList.SetTabGroups(
		types.NewTabGroups(
			"id", "debug", "env", "language", "theme", "color_scheme",
			"asset_url", "title", "login_title", "session_life_time", "bootstrap_file_path", "go_mod_file_path",
			"no_limit_login_ip",
			...
			AddGroup(
				"access_log_off", "access_assets_log_off", "info_log_off", "error_log_off", "sql_log", "logger_level",
				...
				"logger_encoder_encoding", "logger_encoder_time_key", "logger_encoder_level_key",
				"logger_encoder_name_key",
				"logger_encoder_caller_key", "logger_encoder_message_key", "logger_encoder_stacktrace_key",
				"logger_encoder_level",
				"logger_encoder_time", "logger_encoder_duration", "logger_encoder_caller",
			).
			AddGroup(
				"logo", "mini_logo", "custom_head_html", "custom_foot_html", "footer_info", "login_logo",
...
func (s *SystemTable) GetGenerateForm(ctx *context.Context) (generateTool Table) {
		...
		FieldOnChooseAjax(
			...
			func(ctx *context.Context) (success bool, msg string, data interface{}) {
				...
				for i, model := range columnsModel {
					...
					formTypeList[i] = form.GetFormTypeFromFieldType(
	...
	formList.AddField(lgWithScore("table permission", "tool"), "permission", db.Varchar, form.Switch).
		FieldOptions(
			types.FieldOptions{
				{
					Text:  lgWithScore("yes", "tool"),
					Value: "y",
				},
				{
					Text:  lgWithScore("no", "tool"),
					Value: "n",
				},
			},
		).FieldDefault("n")
	...
	formList.AddRow(
		func(panel *types.FormPanel) {
			addSwitchForTool(panel, "filter area", "hide_filter_area", "n", 2)
			panel.AddField(
				lgWithScore("filter form layout", "tool"), "filter_form_layout", db.Varchar, form.SelectSingle,
			).
				FieldOptions(
					types.FieldOptions{
						{
							Text:  form.LayoutDefault.String(),
							Value: form.LayoutDefault.String(),
						},
						{
							Text:  form.LayoutTwoCol.String(),
							Value: form.LayoutTwoCol.String(),
						},
						{
							Text:  form.LayoutThreeCol.String(),
							Value: form.LayoutThreeCol.String(),
						},
						{
							Text:  form.LayoutFourCol.String(),
							Value: form.LayoutFourCol.String(),
						},
						{
							Text:  form.LayoutFlow.String(),
							Value: form.LayoutFlow.String(),
						},
					},
	...
	formList.AddTable(
		lgWithScore("field", "tool"), "fields", func(pa *types.FormPanel) {
			...
			pa.AddField(
				lgWithScore("info field editable", "tool"), "info_field_editable", db.Varchar, form.CheckboxSingle,
			).
				FieldOptions(
					types.FieldOptions{
						{
	...
	formList.AddTable(
		lgWithScore("field", "tool"), "fields_form", func(pa *types.FormPanel) {
			...
			pa.AddField(lgWithScore("field display", "tool"), "field_display", db.Varchar, form.SelectSingle).
				FieldOptions(
					types.FieldOptions{
						{
							Text:  lgWithScore("field display normal", "tool"),
							Value: "0",
						},
						{
							Text:  lgWithScore("field diplay hide", "tool"),
							Value: "1",
						},
						{
							Text:  lgWithScore("field diplay edit hide", "tool"),
							Value: "2",
						},
						{
							Text:  lgWithScore("field diplay create hide", "tool"),
							Value: "3",
						},
					},
				).
			...
			pa.AddField(lgWithScore("form type", "tool"), "field_form_type_form", db.Varchar, form.SelectSingle).
				FieldOptions(formTypeOptions()).FieldDisplay(
				func(value types.FieldModel) interface{} {
					return []string{"Text"}
				},
			)
	...
	formList.AddField(lgWithScore("detail display", "tool"), "detail_display", db.Varchar, form.SelectSingle).
		FieldOptions(
			types.FieldOptions{
				{
					Text:  lgWithScore("follow list page", "tool"),
					Value: "0",
				},
				{
					Text:  lgWithScore("inherit from list page", "tool"),
					Value: "1",
				},
				{
					Text:  lgWithScore("independent from list page", "tool"),
					Value: "2",
				},
			},
		).
	...
	formList.AddTable(
		lgWithScore("field", "tool"), "fields_detail", func(pa *types.FormPanel) {
	...
	formList.SetTabGroups(
		types.
			...
			AddGroup(
				"table_title", "table_description", "hide_filter_area", "filter_form_layout",
			...
			AddGroup(
				"form_title", "form_description", "hide_continue_edit_check_box", "hide_reset_button",
		...
		SetTabHeaders(
			lgWithScore("basic info", "tool"), lgWithScore("table info", "tool"),
...
`
	...
	formList.SetFooterHtml(
		utils.ParseHTML(
			"generator", tmpls["generator"], map[string]string{
	...
	formList.SetWrapper(
		func(content tmpl.HTML) tmpl.HTML {
...
func addSwitchForTool(formList *types.FormPanel, head, field, def string, row ...int) {
	formList.AddField(lgWithScore(head, "tool"), field, db.Varchar, form.Switch).
		FieldOptions(
			types.FieldOptions{
				{
					Text:  lgWithScore("show", "tool"),
					Value: "n",
				},
				{
					Text:  lgWithScore("hide", "tool"),
					Value: "y",
				},
			},
		).FieldDefault(def)
...
func databaseTypeOptions() types.FieldOptions {
	opts := make(
		types.FieldOptions, len(db.IntTypeList)+
